...
osreposinstalled=`apt-get -y update|grep "grizzly/main amd64 Packages"|wc -l`
...
then
	echo ""
	echo "Repositorio de OpenStack Grizzly Instalado - continuando"
else
	echo ""
	echo "Prerequisito inexistente: Repositorio OpenStack Grizzly no instalado"
	echo "Abortando"
	echo ""
	exit 0
fi
...
aptitude -y install pm-utils saidar sysstat iotop ethtool iputils-arping libsysfs2 btrfs-tools \
	...
	glusterfs-client glusterfs-common nfs-client nfs-common
